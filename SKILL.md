...
先按上午、下午或材料中的实际时间顺序组织正文。先建立课堂时间轴，再沿着老师的讲课顺序整理知识点；同一个知识点在后面再次出现时，保留在后面的时间段，并简短说明它与前文的联系。

不要为了合并主题而把后面讲的内容提前，也不要把上午和下午的案例混在同一个主题段落中。只有在最后的“知识关系图”或“课程总结”中，才允许跨时间归纳知识之间的联系。

转写没有明确时间戳时，按材料出现顺序组织，并标注“时间顺序依据：材料顺序”。转写与随堂笔记的顺序冲突时，以带时间戳的转写为准，并在待确认内容中说明冲突。
...
### [后续知识点名称]
[按下午材料的实际顺序继续；与上午相同的概念只说明联系，不提前合并。]
...
- 正文是否先遵守上午、下午或原始材料的时间顺序，再在每个时间段内解释知识点？
- 是否避免为了主题完整，把后面讲的内容或案例提前到前文？
